import {
  ...
  unclipArea,
  valueOrDefault
} from "./chunk-RWXK745O.js";
import {
  src_exports
} from "./chunk-RWPPRX6N.js";
import {
  Adder,
  cubehelix,
  cubehelixLong,
...
function polygonContains_default(polygon, point) {
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m; ++j, lambda02 = lambda1, sinPhi02 = sinPhi1, cosPhi02 = cosPhi1, point0 = point1) {
      ...
      sum.add(atan2(k * sign3 * sin(absDelta), cosPhi02 * cosPhi1 + k * cos(absDelta)));
      angle += antimeridian ? delta + sign3 * tau : delta;
  ...
  function clipLine(stream) {
    ...
    return {
      ...
      point: function(lambda, phi) {
        var point1 = [lambda, phi], point2, v = visible(lambda, phi), c3 = smallRadius ? v ? 0 : code(lambda, phi) : v ? code(lambda + (lambda < 0 ? pi : -pi), phi) : 0;
...
function resample(project, delta2) {
  function resampleLineTo(x05, y05, lambda02, a0, b0, c0, x12, y12, lambda1, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a = a0 + a1, b = b0 + b1, c3 = c0 + c1, m = sqrt(a * a + b * b + c3 * c3), phi2 = asin(c3 /= m), lambda2 = abs(abs(c3) - 1) < epsilon || abs(lambda02 - lambda1) < epsilon ? (lambda02 + lambda1) / 2 : atan2(b, a), p = project(lambda2, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x05, dy2 = y2 - y05, dz = dy * dx2 - dx * dy2;
      if (dz * dz / d2 > delta2 || abs((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
        resampleLineTo(x05, y05, lambda02, a0, b0, c0, x2, y2, lambda2, a /= m, b /= m, c3, depth, stream);
        stream.point(x2, y2);
        resampleLineTo(x2, y2, lambda2, a, b, c3, x12, y12, lambda1, a1, b1, c1, depth, stream);
      }
    }
  }
  return function(stream) {
...
function azimuthalInvert(angle) {
  return function(x, y) {
    var z = sqrt(x * x + y * y), c3 = angle(z), sc = sin(c3), cc = cos(c3);
    return [
      atan2(x * sc, z * cc),
      asin(z && y * sc / z)
    ];
...
// node_modules/chartjs-chart-geo/build/index.js
var lookup$1 = {
...
var ProjectionScale = class extends Scale {
  constructor(cfg) {
    super(cfg);
    this.outlineBounds = null;
    this.oldChartBounds = null;
    this.geoPath = path_default();
  }
  init(options) {
  ...
  computeBounds(outline) {
    const bb = path_default(this.projection.fitWidth(1e3, outline)).bounds(outline);
...
var LegendScale = class extends LinearScale {
  constructor() {
    super(...arguments);
...
var LogarithmicLegendScale = class extends LogarithmicScale {
  constructor() {
    super(...arguments);
    this.legendSize = { w: 0, h: 0 };
  }
...
var colorScaleDefaults = {
  interpolate: "blues",
  missing: "transparent",
  quantize: 0
};
var ColorScale = class extends LegendScale {
  get interpolate() {
    const o = this.options;
    if (!o) {
      return (v) => `rgb(${v},${v},${v})`;
...
ColorScale.id = "color";
ColorScale.defaults = merge2({}, [LinearScale.defaults, baseDefaults, colorScaleDefaults]);
ColorScale.descriptors = {
  _scriptable: (name) => name !== "interpolate",
  _indexable: false
};
var ColorLogarithmicScale = class extends LogarithmicLegendScale {
  constructor() {
    super(...arguments);
...
ColorLogarithmicScale.defaults = merge2({}, [
...
var SizeScale = class extends LegendScale {
  constructor() {
    super(...arguments);
    this._model = null;
  }
  ...
  getSizeImpl(normalized) {
    ...
    if (this.options.mode === "area") {
      ...
      const range3 = a1 - a0;
      const a = normalized * range3 + a0;
      return Math.sqrt(a / Math.PI);
    }
    const range2 = r1 - r0;
    return normalized * range2 + r0;
  }
  _drawIndicator() {
    const { ctx } = this;
    ...
    const positions = labelItems ? labelItems.map((el) => ({ [isHor ? "x" : "y"]: el.options.translation[isHor ? 0 : 1] })) : values.map((_, i) => ({ [isHor ? "x" : "y"]: this.getPixelForTick(i) }));
...
SizeScale.id = "size";
SizeScale.defaults = merge2({}, [LinearScale.defaults, baseDefaults, scaleDefaults]);
SizeScale.descriptors = {
  _scriptable: true,
  _indexable: (name) => name !== "range"
};
var SizeLogarithmicScale = class extends LogarithmicLegendScale {
  constructor() {
    super(...arguments);
    this._model = null;
  }
...
SizeLogarithmicScale.defaults = merge2({}, [LogarithmicScale.defaults, baseDefaults, scaleDefaults]);
...
var GeoFeature = class extends Element {
  constructor() {
    super(...arguments);
    this.cache = void 0;
  }
  inRange(mouseX, mouseY) {
    ...
    const projection2 = this.projectionScale.geoPath.projection();
    if (r && !Number.isNaN(mouseX) && !Number.isNaN(mouseY) && typeof projection2.invert === "function") {
      const longLat = projection2.invert([mouseX, mouseY]);
      return longLat != null && contains_default(this.feature, longLat);
    }
    return r;
  }
  inXRange(mouseX) {
  ...
  _drawInCache(doc) {
    ...
    const x12 = Math.floor(bounds.x);
    const y12 = Math.floor(bounds.y);
    const x2 = Math.ceil(bounds.x + bounds.width);
    const y2 = Math.ceil(bounds.y + bounds.height);
    const pixelRatio = this.pixelRatio || 1;
    const width = Math.ceil(Math.max(x2 - x12, 1) * pixelRatio);
    const height = Math.ceil(Math.max(y2 - y12, 1) * pixelRatio);
    if (width <= 0 || height <= 0) {
      return;
    }
    canvas.width = width;
  ...
  draw(ctx) {
    ...
    if (this.cache && this.cache.canvas && this.cache.canvas.width > 0 && this.cache.canvas.height > 0) {
      ...
      const height = y2 - y12;
      if (width > 0 && height > 0) {
        ctx.drawImage(this.cache.canvas, x12, y12, x2 - x12, y2 - y12);
      }
...
var GeoController = class extends DatasetController {
  getGeoDataset() {
  ...
  update(mode) {
    ...
    if (this.showOutline()) {
      ...
      if (mode !== "resize") {
        ...
        this.updateElement(elem, void 0, properties, mode);
...
var ChoroplethController = class extends GeoController {
  initialize() {
    super.initialize();
...
ChoroplethController.defaults = merge2({}, [
  geoDefaults,
  {
...
ChoroplethController.overrides = merge2({}, [
  geoOverrides,
  {
    plugins: {
      tooltip: {
...
var ChoroplethChart = class extends Chart {
...
var BubbleMapController = class extends GeoController {
  initialize() {
    super.initialize();
    this.enableOptionSharing = true;
  }
  ...
  updateElements(elems, start, count, mode) {
    ...
    for (let i = start; i < start + count; i += 1) {
      ...
      const projection2 = scale.projection([parsed.x, parsed.y]);
      const properties = {
        x: projection2 ? projection2[0] : 0,
        y: projection2 ? projection2[1] : 0,
...
BubbleMapController.defaults = merge2({}, [
  geoDefaults,
  {
...
BubbleMapController.overrides = merge2({}, [
  geoOverrides,
  {
    plugins: {
      tooltip: {
...
var BubbleMapChart = class extends Chart {
  constructor(item, config) {
...
chart.js/dist/helpers.js:
   ...
   *)
*/
//# sourceMappingURL=chartjs-chart-geo.js.map
